...
public class MyApplication extends JFrame {
	...
	public MyApplication() {
		 ...
		 * 		public voud actionPerformed(ActionEvent event) {
		 * 			파일을 불러옵니다.
		 * 		}
		 * }
		 */
		
		// 익명객체 생성하기 - 각각의 이벤트소스에서 사용자와의 상호작용으로 이벤트가 발생했을 때 실행할 코드가 재정의되어 있는 익명객체 생성하기
		ActionListener newFileListener = new ActionListener() {
	    ...
	    ActionListener exitFileListener = new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	           JOptionPane.showMessageDialog(null, "프로그램을 종료합니다..", "종료하기", JOptionPane.WARNING_MESSAGE);
	           System.exit(0);
	        };
	    };
	    
	    // 익명객체로 구현한 이벤트리스너(이벤트 핸들러)객체를 이벤트소스에 등록하기
