...
public class ProductController {
	 ...
	 * 		+ 객체조립(의존성 주입)절차
	 * 		  1. 스프링 컨테이너가 빈 설정 정보를 이용해서 해당 객체를 모두 생성하고, 스프링 컨터이너에 보관한다.
	 * 		  2. AnnotaionProcessor가 스프링 컨테이너에 등록된 모든 객체의 어노테이션 정보를 분석한다.
	 *  	  3. @Autowired, @Resource 어노테이션을 감지하면 해당 어노테이션이 부착된 멤버변수의 타입을 알아낸다.
	 *  	  4. 스프링 컨테이너에 보관된 객체들 중에서 해당 타입의 객체가 있는지 확인한다. 
	 *  	  5. 해당 타입의 객체가 발견되면 그 객체의 참조값을 멤버변수에 대입해서 객체가 조립되게 한다.
	 *           단, 해당 타입의 객체가 존재하지 않거나, 해당 타입의 객체가 2개 이상 발견되는 경우 예외가 발생한다.		
	 *           
