...
public class BookInsertForm {
	...
	// @DateTimeFormat
	// 입력필드에서 선택 혹은 입력한 날짜형식과 같은 형식으로 패턴을 정의하면 요청파라미터에서 값을 조회해서 java.util.Date타입으로 변환해서 pubDate에 저장시킨다.
	@DateTimeFormat(pattern = "yyyy-MM-dd")
	private Date pubDate;
	private int stock;
	// 2개 이상의 첨부파일이 같은 이름을 업로드될 때는 배열 혹은 콜렉션 타입의 변수를 선언한다.
	// MultipartFile객체는 업로드된 첨부파일의 정보를 제공하는 객체다.
	// MultipartFile객체는 첨부파일의 유무, 파일명, 파일 사이즈, 파일의 컨텐츠 타입, 파일 데이터, 파일과 연결된 스트림을 제공한다.
