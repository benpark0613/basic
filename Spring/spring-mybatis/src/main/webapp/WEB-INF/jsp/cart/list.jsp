...
<div class="container">
	...
	<div class="modal fade" id="modal-info-book" tabindex="-1" aria-labelledby="책 상세정보 모달" aria-hidden="true">
					...
					<table class="table">
						<tbody>
							...
							<tr>
								...
								<th>할인가격</th>
								<td><span id="span-book-discount-price" class="fw-bolder text-danger">32,000</span> 원</td>
							</tr>
							<tr>
								<th>출판일자</th>
	...
	<div class="modal fade" id="modal-info-error" tabindex="-1" aria-labelledby="오류 메세지 모달창" aria-hidden="true">
  		<div class="modal-dialog">
    		<div class="modal-content">
      			...
      			<div class="modal-body">
      				<p class="text-danger"><strong>오류</strong> <span id="span-error">오류 메세지가 표시된다.</span></p>
      			</div>
      			<div class="modal-footer">
        			<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">닫기</button>
      			</div>
...
// $(function() { 스트립트 코딩 })은 HTML DOM객체가 준비되면 즉시 코딩된 스크립트가 실행된다.(HTML DOM이 화면에 렌더링되기 전에 미리 스크립트작업이 완료된다)
// 즉, 엘리먼트에 이벤트핸들러 함수를 등록하는 것, 특정 엘리먼트를 감추는 것, 서버와 통신해서 초기화면에 필요한 데이터를 획득하는 작업 등 다양한 작업을
// HTML DOM이 화면에 렌더링되기 전에 수행할 수 있다.
// 함수 안에서 선언한 모든 변수, 함수, 객체들은 그 함수의 범위를 벗어나지 않는다. 즉, 브라우저 전역객체(Window객체)을 오염시키지 않는다.
// 사용자가 정의한 변수나 함수가 이미 다른 사용자가 정의한 변수나 함수를 덮어쓸 가능성은 없앤 준다.
...
$(function() {
	...
	// 장바구니 테이블안에 있는 a링크를 클릭했을 때 실행될 이벤트핸들러함수를 등록한다.
	$("#table-cart-items a").click(function(event) {
		event.preventDefault();	// a 태그에서 클릭이벤트가 발생하면 링크된 페이지도 이동하는 기본 동작이 일어나지 않게한다.
		...
		// 서버로부터 성공적인 응답이 오면 function(데이터) { ... } 함수가 실행된다.
		// jQuery는 서버로 부터 성공적인 응답이 오면 응답컨텐츠(json 형식의 텍스트)를 자바스크립트 객체나 배열로 변환한 다음, 함수를 실행할 때 전달한다.
		$.getJSON('/rest/book/detail.do', {no:bookNo}, function(book) {
			...
			$("#span-book-discount-price").text(book.discountPrice.toLocaleString());
	...
	// 삭제버튼을 클릭했을 때 실행될 이벤트핸들러 함수를 등록한다.
	$("#table-cart-items button.btn-outline-danger").click(function() {
		...
		$.getJSON("/rest/cart/delete.do", {no:itemNo}, function(response) {
			/*
				장바구니에서 아이템 삭제가 성공했을 때 응답 데이터
				response = {status:"OK", error:null, items:[5]}
	
				장바구니에서 아이템 삭제가 실패했을 때 응답 데이터
				response = {status:"FAIL", error:"다른 사용자의 아이템을 삭제할 수 없습니다.", items:null}
			*/
			if (response.status == 'OK') {
				$btn.closest('tr').remove();
				// 내비바 장바구니 메뉴의 아이템 갯수를 갱신한다.
