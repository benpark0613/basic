...
public class ExampleSample2 {
	...
	public static void main(String[] args) {
		 ...
		 * 			* 유효한 입력값이 아닌 경우
		 * 				(아이디가 빈 문자열인 것, 이메일형식이 아닌 이메일주소, 값의 범위를 넘어가는 값, 제시된 항목이 아닌 항목의 값)
		 * 		- 예외 강제 발생시키기
		 * 			throw new 예외클래스();
		...
		try {
			 ...
			 * 			+ 오류가 발생한 수행문을 확인할 수 있다.   예외객체의 void printStackTrace()는 예외가 발생한 수행문정보를 출력한다.
			 */
		...
		public void addNewUser(User user) {
			...
			for (User savedUser : userList) {
				if (savedUser.getId().equals(user.getId())) {
					// 아이디 중복상황 확인, 예외 강제발생, 예외가 발생하면 해당 메소드는 즉시 실행이 중단된다. 빠른 종료에 해당된다.
					throw new RuntimeException("["+user.getId()+"] 아이디 중복");
				}
				if (savedUser.getName().equals(user.getName())) {
					// 이름 중복상황 확인, 예외 강제발생, 예외가 발생하면 해당 메소드는 즉시 실행이 중단된다 빠른 종료에 해당된다.
					throw new RuntimeException("["+user.getName()+"] 이름 중복");
				}				
			}
			// userList객체에 User 객체를 저장하는 이 수행문이 실행되기 위해서는 for문의 예외를 강제로 발생시키는 수행문이 절대로 수행되지 않아야 한다.
			userList.add(user);
