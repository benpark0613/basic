...
<div class="container">    
	...
	<div class="row mb-3">
		<div class="col">
			...
			<%--
				cart1이라는 속성명으로 조회한 값이 null이다.
				empty 연산자는 속성값이 null이면 true를 반환한다.
			 --%>
			 
			<p>${empty cart2 } : ${cart2 }</p>
			<%--
				cart2이라는 속성명으로 조회한 값이 길이가 0인 리스트다.(비어있는 리스트다.)
				empty 연산자는 속성값이 비어있는 List인 경우 true를 반환한다.
			 --%>
			 
			<p>${empty cart3 } : ${cart3 }</p>
			<%--
				cart3이라는 속성명으로 조회한 값이 객체를 여러개 포함하고 있는 리스트다.
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td class="fw-bold ${cart3[1].discountRate > 0 ? 'text-danger' : '' }">${cart3[1].discountPrice } 원</td>
					...
					<tr>
						...
						<td class="fw-bold ${cart3[2].discountRate > 0 ? 'text-danger' : '' }">${cart3[2].discountPrice } 원</td>
