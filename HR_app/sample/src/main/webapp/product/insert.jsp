...
<%
	/*
		...
		Tomcat은 요청메세지정보를 저장하는 요청객체(HttpServletRequest)를 생성한다.
		Tomcat은 요청메세지를 분석해서 요청헤더정보와 요청파라미터정보(폼입력값, 요청처리에 필요한 값)를 요청객체에 저장한다.
		Tomcat은 insert.jsp의 _jspService(request, response)메소드를 실행하면서 request 매개변수에 요청객체를 전달한다.
	...
	*/
	...
	System.out.println("### form.jsp가 insert.jsp로 제출한 값");
	...
	/*
		...
		// 저장이 완료되면 complete.jsp를 재요청하는 응답을 브라우저에게 보낸다.
		response.sendRedirect("complete.jsp");
	*/
	
	// complete.jsp를 재요청하게하는 응답을 브라우저에게 보낸다.
	response.sendRedirect("complete.jsp");
